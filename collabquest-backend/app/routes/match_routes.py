...
class MatchResponse(BaseModel):
    ...
    project_name: str
    status: str
    rejected_by: Optional[str] = None # <--- Added this
...
async def create_match(user_id: str, project_id: str, leader_id: str):
    ...
    try:
        ...
        await Notification(recipient_id=user_id, sender_id=leader_id, message=f"You matched with {p_name}!", type="match", related_id=project_id).insert()
...
async def match_teammates_for_user(project_id: Optional[str] = None, current_user: User = Depends(get_current_user)):
    ...
    if project_id:
        ...
        if target_project:
            for member_id in target_project.members: exclude_ids.add(member_id)
    else:
        my_teams = await Team.find(Team.members == str(current_user.id)).to_list()
        for team in my_teams:
            for member_id in team.members: exclude_ids.add(member_id)
...
async def handle_swipe(data: SwipeRequest, current_user: User = Depends(get_current_user)):
    try:
        if not data.target_id or data.target_id == "[object Object]": return {"status": "error"}
        ...
        if data.direction == "left": return {"status": "passed", "is_match": False}
        is_match = False
        if data.type == "project":
            project = await Team.get(data.target_id)
            if project and project.members:
                leader_id = project.members[0]
                reverse_swipe = await Swipe.find_one(Swipe.swiper_id == leader_id, Swipe.target_id == str(current_user.id), Swipe.direction == "right", Or(Swipe.related_id == str(project.id), Swipe.related_id == None))
                if reverse_swipe:
                    is_match = True
    ...
    for m in matches_as_candidate:
        try:
            ...
            if leader and project:
                results.append({
                    "id": str(leader.id), "name": leader.username, "avatar": leader.avatar_url or "https://github.com/shadcn.png", "contact": leader.email, "role": "Team Leader", "project_id": str(project.id), "project_name": project.name, "status": m.status,
    ...
    for m in matches_as_leader:
        try:
            ...
            if candidate and project:
                results.append({
                    "id": str(candidate.id), "name": candidate.username, "avatar": candidate.avatar_url or "https://github.com/shadcn.png", "contact": candidate.email, "role": "Teammate", "project_id": str(project.id), "project_name": project.name, "status": m.status,
                    "rejected_by": m.rejected_by
                })
        except: continue
    return results