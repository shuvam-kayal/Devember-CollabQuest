...
from app.models import Team, User, Notification, Match, ChatGroup
...
class TeamDetailResponse(BaseModel):
    ...
    chat_group_id: Optional[str] = None # <--- New Field
...
async def get_team_details(team_id: str):
    ...
    return {
        ...
        "chat_group_id": str(chat_group.id) if chat_group else None # <--- Return ID
    }
...
async def add_member(team_id: str, req: InviteRequest, current_user: User = Depends(get_current_user)):
    ...
    if candidate_id not in team.members:
        ...
        # Only notify leader if leader didn't trigger this action (e.g. if user accepted invite)
