...
async def update_team_details(team_id: str, data: TeamUpdate, current_user: User = Depends(get_current_user)):
    ...
    if data.active_needed_skills is not None: 
        team.active_needed_skills = data.active_needed_skills
    if data.is_looking_for_members is not None:
        team.is_looking_for_members = data.is_looking_for_members
    if data.status:
...
async def add_member(team_id: str, req: InviteRequest, current_user: User = Depends(get_current_user)):
    ...
    if candidate_id not in team.members:
        ...
        # --- AUTO DISABLE RECRUITMENT LOGIC ---
...
async def get_team_tasks(team_id: str):
    ...
    for t in team.tasks:
        ...
        results.append({
            "id": t.id, "description": t.description, "assignee_id": t.assignee_id,
            "assignee_name": assignee.username if assignee else "Unknown",
            "assignee_avatar": assignee.avatar_url, "deadline": t.deadline,
            "status": t.status, "verification_votes": len(t.verification_votes),
            "required_votes": math.ceil(len(team.members) * 0.2),
            "rework_votes": len(t.rework_votes), "required_rework": math.ceil(len(team.members) * 0.3),
