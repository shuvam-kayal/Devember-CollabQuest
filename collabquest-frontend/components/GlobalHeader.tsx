...
export default function GlobalHeader() {
    ...
    try {
      ...
      ws.onmessage = (event) => { 
          ...
          if (data.event === "notification") {
              ...
              // FIX: Refresh Dashboard/Team page WITHOUT adding unread count
              window.dispatchEvent(new Event("dashboardUpdate"));
          }
          else {
  ...
  const toggleNotifications = async () => {
      ...
      if (newState) {
          const token = Cookies.get("token");
          // Keep action items unread until acted upon
  ...
  // ... (handleVote, handleAccept, handleReject same as before) ...
  const handleVote = async (notif: Notification, decision: 'approve' | 'reject') => {
      ...
      const endpoint = notif.type === 'completion_request' 
          ? `http://localhost:8000/teams/${notif.related_id}/complete/vote`
          : `http://localhost:8000/teams/${notif.related_id}/delete/vote`;

      try {
          const res = await axios.post(endpoint, { decision }, { headers: { Authorization: `Bearer ${token}` } });
          ...
          if (res.data.status === "deleted") {
              ...
              setNotifications(prev => prev.map(n => n._id === notif._id ? { ...n, is_read: true, action_status: "accepted" } : n)); // Use "accepted" visually for "Complete"
              window.dispatchEvent(new Event("dashboardUpdate"));
