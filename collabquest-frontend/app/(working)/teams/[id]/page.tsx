...
export default function TeamDetails() {
    ...
    const handleLikeUser = async (user: SearchResultUser) => {
        ...
        try {
            ...
            const res = await api.post("/matches/swipe", { target_id: safeTargetId, direction: "right", type: "user", related_id: teamId });
            if (res.data.is_match) { alert(`IT'S A MATCH! 🎉\nYou matched with ${user.username}.`); fetchCandidates(teamId); }
        } catch (e) {
    ...
    const askAiForStack = async () => {
        ...
        if (!descToCheck.trim()) { alert("⚠️ Missing Description\nPlease add a project description."); return; }
        ...
        } catch (err: any) { const msg = err.response?.data?.detail || "AI Suggestion failed."; alert(msg); } finally { setIsSuggesting(false); }
    };
    ...
    const generateRoadmap = async () => {
        if (!team?.description?.trim()) { alert("⚠️ Missing Description"); return; }
        if (!team?.needed_skills || team.needed_skills.length === 0) { alert("⚠️ Missing Tech Stack"); return; }
    ...
    const hasActiveVotes = team.member_requests?.some(r => r.is_active) || team.deletion_request?.is_active || team.completion_request?.is_active;
            ...
            <div className="relative z-40 max-w-7xl mx-auto p-4 md:p-8">
                    ...
                    <div className="flex flex-col lg:flex-row justify-between items-start gap-8">
                        <div className="flex-1 space-y-4">
                            <div className="flex items-center gap-3">
                                {isRecruiting ? <span className="animate-pulse inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-green-500/10 text-green-400 border border-green-500/20 shadow-[0_0_10px_rgba(34,197,94,0.2)]"><Megaphone className="w-3 h-3" /> Recruiting</span> : <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider bg-zinc-800 text-zinc-500 border border-zinc-700">Filled</span>}
                            ...
                            <h1 className="text-5xl md:text-6xl font-black tracking-tight text-white mb-2">{team.name} {isLeader && team.status !== "completed" && <Link href={`/teams/${team.id}/edit`} className="inline-block ml-4 align-middle"><Edit2 className="w-5 h-5 text-zinc-600 hover:text-purple-400 transition" /></Link>}</h1>
                        ...
                        <div className="flex flex-col gap-3 w-full lg:w-auto min-w-[200px]">
                            {isLeader && team.status !== "completed" && (
                                <>
                                    <button onClick={() => setShowRecruitModal(true)} className="w-full px-6 py-3.5 bg-zinc-100 text-black hover:bg-white rounded-xl font-bold flex items-center justify-center gap-2 transition-all active:scale-95 shadow-[0_0_20px_rgba(255,255,255,0.1)]">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
                    <div className="md:col-span-2 bg-zinc-900/60 backdrop-blur-xl border border-white/5 p-8 rounded-[2rem] relative overflow-hidden group">
                        ...
                        <div className="flex justify-between items-center mb-6">
                            ...
                            {isEditingSkills && <div className="flex gap-2"><button onClick={askAiForStack} disabled={isSuggesting} className="text-xs bg-blue-600/20 text-blue-400 border border-blue-500/30 px-3 py-1.5 rounded-lg flex gap-1.5 items-center hover:bg-blue-600 hover:text-white transition">{isSuggesting ? <Loader2 className="w-3 h-3 animate-spin" /> : <Sparkles className="w-3 h-3" />} AI Suggest</button><button onClick={saveSkills} className="text-xs bg-green-600 text-white px-4 py-1.5 rounded-lg font-bold hover:bg-green-500">Save</button></div>}
                        </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {(isEditingSkills ? localSkills : team.needed_skills).map((skill, k) => <span key={k} className="bg-zinc-900/50 text-zinc-300 px-4 py-2 rounded-xl text-sm border border-zinc-700/50 flex items-center gap-2 shadow-sm hover:border-zinc-500 transition-colors">{skill}{isEditingSkills && <button onClick={() => removeSkill(skill)}><X className="w-3 h-3 text-zinc-500 hover:text-red-400" /></button>}</span>)}
                            {isEditingSkills && <div className="relative"><select className="bg-black/50 border border-zinc-700 text-zinc-400 text-sm rounded-xl px-4 py-2 outline-none focus:border-purple-500 appearance-none pr-8 cursor-pointer hover:bg-zinc-900" value={dropdownValue} onChange={(e) => addSkill(e.target.value)}><option value="" disabled>+ Add Skill</option>{PRESET_SKILLS.filter(s => !localSkills.includes(s)).map(s => <option key={s} value={s}>{s}</option>)}</select><Plus className="absolute right-3 top-2.5 w-4 h-4 text-zinc-500 pointer-events-none" /></div>}
                        </div>
                    </div>

                    <div className="bg-zinc-900/60 backdrop-blur-xl border border-white/5 p-8 rounded-[2rem] flex flex-col h-full">
                        <h3 className="text-xl font-bold flex items-center gap-3 text-white mb-6"><Users className="text-green-400 w-6 h-6" /> Team</h3>
                            ...
                            {team.members.map((m: any, i: number) => {
                                ...
                                return (
                                    <div key={memberId || i} className="flex items-center justify-between group p-2 hover:bg-white/5 rounded-xl transition-all">
                                        <Link href={`/profile/${memberId}`} className="flex items-center gap-3 cursor-pointer">
                                            <img src={m.avatar_url || "https://github.com/shadcn.png"} className={`w-10 h-10 rounded-full bg-zinc-800 object-cover ${(m.id === team.leader_id || m._id === team.leader_id) ? "border-2 border-yellow-400 shadow-[0_0_15px_rgba(250,204,21,0.5)]" : "border border-zinc-700"}`} />
                                            <div><p className="text-sm font-bold text-zinc-200 group-hover:text-purple-400 transition leading-tight">{m.username}</p>{(m.id === team.leader_id || m._id === team.leader_id) && <span className="flex items-center gap-1 text-[9px] font-black text-yellow-500 bg-yellow-500/10 px-2 py-0.5 rounded border border-yellow-500/20 mt-1 uppercase tracking-wide"><Trophy className="w-3 h-3" /> Leader</span>}</div>
                                        </Link>
                                        <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                            {(isMember && memberId !== currentUserId) || (!isMember && (m.id === team.leader_id || m._id === team.leader_id)) ? <><Link href={`/chat?targetId=${memberId}`}><button className="text-zinc-500 hover:text-blue-400 p-2 hover:bg-blue-500/10 rounded-lg transition"><MessageSquare className="w-4 h-4" /></button></Link><button onClick={() => openEmailComposer(m)} className="text-zinc-500 hover:text-green-400 p-2 hover:bg-green-500/10 rounded-lg transition"><Mail className="w-4 h-4" /></button></> : null}
                                            {isLeader && memberId !== team.leader_id && team.status !== "completed" && <button onClick={() => handleRemove(memberId)} className="text-zinc-500 hover:text-red-500 p-2 hover:bg-red-500/10 rounded-lg transition"><Trash2 className="w-4 h-4" /></button>}
                                        </div>
                                    </div>
                                );
                            })}
                    ...
                    <div className="mb-20">
                        ...
                        {isLeader && (
                            <div className="bg-[#0f0f0f] border border-white/5 p-6 rounded-[2.5rem] mb-8 space-y-4 shadow-2xl shadow-black/50">
                                <input className="w-full bg-black border border-white/10 rounded-2xl p-4 text-sm outline-none focus:border-purple-500 transition-all shadow-inner text-white placeholder:text-zinc-600" placeholder="What needs to be done?" value={newTaskDesc} onChange={e => setNewTaskDesc(e.target.value)} />
                                <div className="flex flex-col md:flex-row gap-3">
                                    <div className="relative flex-1">
                                        <select className="w-full h-full bg-black border border-white/10 rounded-2xl p-4 text-sm outline-none appearance-none text-zinc-300 cursor-pointer" value={newTaskAssignee} onChange={e => setNewTaskAssignee(e.target.value)}><option value="">Select Collaborator</option>{team.members.map(m => <option key={m.id} value={m.id}>{m.username}</option>)}</select>
                                        <ChevronRight className="absolute right-4 top-1/2 -translate-y-1/2 w-4 h-4 rotate-90 text-zinc-600 pointer-events-none" />
                                    </div>
                                    <input type="datetime-local" className="flex-1 bg-black border border-white/10 rounded-2xl p-4 text-sm text-zinc-300 outline-none" style={{ colorScheme: 'dark' }} value={newTaskDeadline} onChange={e => setNewTaskDeadline(e.target.value)} />
                                    <button onClick={handleAssignTask} disabled={isTaskLoading} className="bg-purple-600 hover:bg-purple-500 rounded-2xl px-8 font-black uppercase text-[10px] tracking-widest shadow-lg shadow-purple-500/20 transition-all hover:scale-105 active:scale-95 text-white">{isTaskLoading ? <Loader2 className="w-4 h-4 animate-spin" /> : "Assign"}</button>
                                </div>
                            </div>
                        )}
                            ...
                            {team?.members.map(member => {
                                ...
                                return (
                                    <div key={member.id} className="bg-zinc-900/30 border border-zinc-800 rounded-xl p-4">
                                        <div className="flex items-center gap-2 mb-4 border-b border-zinc-800 pb-2"><img src={member.avatar_url || "https://github.com/shadcn.png"} className="w-6 h-6 rounded-full" /><h3 className="font-bold text-zinc-300">{member.username}'s Tasks</h3></div>
                                        <div className="space-y-3">
                                            {memberTasks.map(task => {
                                                const isMyTask = task.assignee_id === currentUserId;
                                                const timeLeft = new Date(task.deadline).getTime() - Date.now();
                                                let style = { container: "border-blue-500/30 bg-blue-900/10", iconBox: "bg-blue-500/20 text-blue-400", text: "text-blue-100", metaText: "text-blue-400/60", icon: <Clock className="w-5 h-5" /> };
                                                if (task.status === 'completed') style = { container: "border-zinc-800 bg-zinc-900/30 opacity-60", iconBox: "bg-zinc-800 text-zinc-500", text: "text-zinc-500 line-through", metaText: "text-zinc-600", icon: <CheckCircle2 className="w-5 h-5" /> };
                                                else if (task.status === 'rework') style = { container: "border-red-500/50 bg-red-900/10", iconBox: "bg-red-500/20 text-red-400", text: "text-red-200", metaText: "text-red-400/60", icon: <RotateCcw className="w-5 h-5" /> };
                                                ...
                                                return (
                                                    <div key={task.id} className={`border p-4 rounded-xl flex flex-col md:flex-row items-center justify-between gap-4 transition-all hover:scale-[1.01] ${style.container}`}>
                                                        ...
                                                        <div className="flex items-center gap-2">
                                                            {(task.status === 'pending' || task.status === 'rework') && isMyTask && <button onClick={() => handleSubmitTask(task.id)} className="bg-blue-600 hover:bg-blue-500 text-white px-4 py-2 rounded-lg text-xs font-bold uppercase tracking-widest shadow-lg shadow-blue-900/20 transition-all active:scale-95">Deliver</button>}
                                                            {task.status === 'review' && !isMyTask && <div className="flex gap-2"><button onClick={() => handleVerifyTask(task.id)} className="bg-green-600 hover:bg-green-500 text-white px-3 py-2 rounded-lg text-xs font-bold flex items-center gap-1 shadow-lg shadow-green-900/20"><Check className="w-3 h-3" /> Verify</button><button onClick={() => handleReworkTask(task.id)} className="bg-zinc-800 hover:bg-red-500 text-zinc-400 hover:text-white px-3 py-2 rounded-lg text-xs font-bold flex items-center gap-1 transition-colors"><RotateCcw className="w-3 h-3" /> Rework</button></div>}
                                                            {task.is_overdue && isMyTask && !task.extension_active && <button onClick={() => openExtensionModal(task.id)} className="text-[10px] font-black text-blue-400 bg-blue-400/10 px-3 py-1.5 rounded-xl border border-blue-400/20 hover:bg-blue-400/20 transition">Ask Extension</button>}
                                                            {task.extension_active && !isMyTask && <div className="flex gap-1"><button onClick={() => handleVoteExtension(task.id, 'approve')} className="bg-green-600/20 text-green-400 hover:bg-green-600 hover:text-white p-1.5 rounded transition"><Check className="w-3 h-3" /></button><button onClick={() => handleVoteExtension(task.id, 'reject')} className="bg-red-600/20 text-red-400 hover:bg-red-600 hover:text-white p-1.5 rounded transition"><X className="w-3 h-3" /></button></div>}
                                                            {isLeader && <button onClick={() => handleDeleteTask(task.id)} className="text-zinc-600 hover:text-red-500 p-2 transition"><Trash2 className="w-4 h-4" /></button>}
                                                        </div>
                                                    </div>
                                                );
                                            })}
                ...
                <div className="mb-16">
                    <div className="flex items-center justify-between mb-8">
                        ...
                        {isLeader && team.project_roadmap && team.project_roadmap.phases && <button onClick={generateRoadmap} disabled={isGenerating} className="text-xs flex items-center gap-2 px-3 py-1.5 rounded-lg border border-zinc-800 text-zinc-400 hover:text-white hover:bg-zinc-900 transition">{isGenerating ? <Loader2 className="w-3 h-3 animate-spin" /> : <RefreshCw className="w-3 h-3" />} Regenerate</button>}
                    </div>
                    {!team.project_roadmap || !team.project_roadmap.phases ? (
                        <div className="text-center py-20 bg-zinc-900/20 rounded-3xl border border-zinc-800/50 border-dashed">
                            ...
                            {isLeader ? <button onClick={generateRoadmap} disabled={isGenerating} className="px-8 py-4 bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 rounded-xl font-bold transition flex items-center gap-2 mx-auto shadow-lg shadow-purple-900/20">{isGenerating ? <Loader2 className="animate-spin" /> : <Sparkles className="w-5 h-5" />} Generate Plan</button> : <p className="text-sm text-zinc-600">Waiting for leader initialization.</p>}
                        </div>
                    ) : (
            ...
            <AnimatePresence>
                ...
                {showRecruitModal && (
                    <div className="fixed inset-0 bg-black/90 flex items-center justify-center z-50 p-4 backdrop-blur-md">
                        <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.95, opacity: 0 }} className="bg-zinc-900 border border-zinc-800 w-full max-w-3xl rounded-[2rem] shadow-2xl flex flex-col max-h-[90vh]">
                            <div className="p-6 border-b border-zinc-800 flex justify-between items-center bg-zinc-950/50 rounded-t-[2rem]">
                                <div><h2 className="text-2xl font-bold text-white flex items-center gap-2"><UserPlus className="w-6 h-6 text-purple-500" /> Recruit Talent</h2><p className="text-zinc-400 text-sm mt-1">Find teammates for <strong>{team.name}</strong></p></div>
                                <button onClick={() => setShowRecruitModal(false)} className="p-2 hover:bg-zinc-800 rounded-full transition"><X className="w-6 h-6 text-zinc-400" /></button>
                            </div>
                            <div className="p-6 border-b border-zinc-800 bg-zinc-900">
                                <button onClick={() => router.push(`/matches?type=users&projectId=${teamId}`)} className="w-full mb-6 bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-500 hover:to-blue-500 text-white p-4 rounded-xl font-bold text-lg shadow-lg flex items-center justify-center gap-2 transition-transform hover:scale-[1.02] active:scale-[0.98]"><Sparkles className="w-6 h-6 text-yellow-300" /> Launch Smart Finder (Swipe Mode)</button>
                                <div className="flex items-center gap-2 mb-3"><span className="text-xs font-bold uppercase text-zinc-500 tracking-wider">Or search manually:</span><div className="h-px bg-zinc-800 flex-1"></div></div>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="relative"><Search className="absolute left-3 top-3 w-5 h-5 text-zinc-500" /><input className="w-full bg-black border border-zinc-800 rounded-xl py-3 pl-10 pr-4 outline-none focus:border-purple-500 transition text-sm" placeholder="Search by username..." value={recruitSearch} onChange={(e) => setRecruitSearch(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && handleSearchCandidates()} /></div>
                                    <div className="relative"><Filter className="absolute left-3 top-3 w-5 h-5 text-zinc-500" /><select className="w-full bg-black border border-zinc-800 rounded-xl py-3 pl-10 pr-4 outline-none focus:border-purple-500 transition text-sm appearance-none text-zinc-300" value={recruitSkillFilter} onChange={(e) => setRecruitSkillFilter(e.target.value)}><option value="">Filter by Skill...</option>{PRESET_SKILLS.map(skill => (<option key={skill} value={skill}>{skill}</option>))}</select></div>
                                    <div className="md:col-span-2 flex justify-end"><button onClick={handleSearchCandidates} className="bg-zinc-800 hover:bg-zinc-700 text-white px-6 py-2 rounded-lg font-bold text-sm flex items-center gap-2 transition border border-zinc-700">{isSearching ? <Loader2 className="w-4 h-4 animate-spin" /> : <Search className="w-4 h-4" />} Search</button></div>
                                </div>
                            </div>
                            <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-zinc-950">
                                {isSearching ? <div className="flex flex-col items-center justify-center py-20 text-zinc-500"><Loader2 className="w-10 h-10 animate-spin mb-4 text-purple-500" /><p>Searching...</p></div> : recruitResults.length === 0 ? <div className="text-center py-20 text-zinc-600"><UserPlus className="w-12 h-12 mx-auto mb-4 opacity-20" /><p>No candidates found.</p></div> : recruitResults.map((user) => (
                                    <div key={user.id} className="bg-zinc-900 border border-zinc-800 p-4 rounded-xl flex flex-col md:flex-row items-center justify-between gap-4 hover:border-zinc-700 transition group">
                                        <Link href={`/profile/${user.id}`} target="_blank" className="flex items-center gap-4 w-full md:w-auto flex-1 cursor-pointer">
                                            <img src={user.avatar_url || "https://github.com/shadcn.png"} className="w-12 h-12 rounded-full border border-zinc-800" />
                                            <div>
                                                <div className="flex items-center gap-2"><h4 className="font-bold text-white text-lg hover:text-purple-400 transition">{user.username}</h4><span className="text-xs bg-purple-900/30 text-purple-400 px-2 py-0.5 rounded border border-purple-500/20 font-mono">{user.match_score}% Match</span></div>
                                                <div className="flex flex-wrap gap-1 mt-1">{Array.isArray(user.skills) && user.skills.slice(0, 4).map((s: any, idx: number) => <span key={idx} className="text-[10px] bg-zinc-800 text-zinc-400 px-2 py-0.5 rounded-full">{typeof s === 'string' ? s : s.name}</span>)}{Array.isArray(user.skills) && user.skills.length > 4 && <span className="text-[10px] text-zinc-500">+{user.skills.length - 4}</span>}</div>
                                            </div>
                                        </Link>
                                        <div className="flex items-center gap-3 w-full md:w-auto justify-end">
                                            <Link href={`/profile/${user.id}`} target="_blank"><button className="p-2 text-zinc-500 hover:text-white transition" title="View Profile"><ExternalLink className="w-5 h-5" /></button></Link>
                                            <button onClick={() => handleLikeUser(user)} disabled={user.has_invited} className={`px-5 py-2 rounded-lg font-bold text-sm flex items-center gap-2 transition shadow-lg ${user.has_invited ? "bg-zinc-800 text-zinc-400 cursor-not-allowed border border-zinc-700" : "bg-purple-600 hover:bg-purple-500 text-white shadow-purple-900/20"}`}>{user.has_invited ? <Check className="w-4 h-4" /> : <ThumbsUp className="w-4 h-4" />} {user.has_invited ? "Liked" : "Like"}</button>
                                        </div>
                                    </div>
                                ))}
                            </div>
