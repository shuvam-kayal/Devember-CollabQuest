...
export default function Dashboard() {
  ...
  const fetchUserProfile = async (jwt: string) => { 
      try { const response = await axios.get("http://localhost:8000/users/me", { headers: { Authorization: `Bearer ${jwt}` } }); setUser(response.data); } catch (error) { Cookies.remove("token"); router.push("/"); } 
  };
  const fetchMatches = async (jwt: string) => { 
      try { const res = await axios.get("http://localhost:8000/matches/mine", { headers: { Authorization: `Bearer ${jwt}` } }); setProjectOpportunities(res.data.filter((m: any) => m.role === "Team Leader").reverse()); } catch (e) { } 
  }
  const fetchMyProjects = async (jwt: string) => {
      try { const res = await axios.get("http://localhost:8000/teams/", { headers: { Authorization: `Bearer ${jwt}` } }); const uid = (await axios.get("http://localhost:8000/users/me", { headers: { Authorization: `Bearer ${jwt}` } })).data._id; setMyProjects(res.data.filter((t: any) => t.members[0] === uid)); } catch (e) { }
  }
  ...
  const handleDeleteMatch = async (match: Match) => {
    ...
    try {
        ...
        await axios.delete(`http://localhost:8000/matches/delete/${match.project_id}/${myId}`, {
  ...
  const handleConnectionAction = async (match: Match) => { 
      ...
      try { 
          ...
          await axios.post(`http://localhost:8000/teams/${match.project_id}/members`, { target_user_id: target }, { headers: { Authorization: `Bearer ${token}` } }); 
