...
import {
    ...
    Sparkles, X, Plus, RefreshCw, Trash2, Check, AlertTriangle, MessageSquare, Mail, ThumbsUp, XCircle, Clock,
    Send, Edit2, Save, Users, Trophy, Star, Megaphone, Play, LogOut, UserMinus, Timer, CalendarClock,
    Layout, List, AlertCircle, ChevronLeft, ChevronRight, Briefcase, Settings
} from "lucide-react";
...
export default function TeamDetails() {
    ...
    const [user, setUser] = useState<UserProfile | null>(null); // Full user object for sidebar
        ...
        const init = async () => {
            try {
                ...
                setUser({
                    ...
                    avatar_url: userData.avatar_url || "https://github.com/shadcn.png",
    ...
    const isLeader = team && user && user.id === team.leader_id;
    ...
    const handleInitiateDelete = async () => { if (!confirm("Start vote?")) return; setDeletionProcessing(true); try { const res = await api.post(`/teams/${teamId}/delete/initiate`, {}); if (res.data.status === "deleted") router.push("/dashboard"); else fetchTeamData(); } catch (e) { } finally { setDeletionProcessing(false); } };
    const handleVoteDelete = async (decision: 'approve' | 'reject') => { setDeletionProcessing(true); try { const res = await api.post(`/teams/${teamId}/delete/vote`, { decision }); if (res.data.status === "deleted") router.push("/dashboard"); else fetchTeamData(); } catch (e) { } finally { setDeletionProcessing(false); } };
    const handleInitiateComplete = async () => { if (!confirm("Mark project completed?")) return; setCompletionProcessing(true); try { await api.post(`/teams/${teamId}/complete/initiate`, {}); fetchTeamData(); } catch (e) { } finally { setCompletionProcessing(false); } }
    const handleVoteComplete = async (decision: 'approve' | 'reject') => { setCompletionProcessing(true); try { await api.post(`/teams/${teamId}/complete/vote`, { decision }); fetchTeamData(); } catch (e) { } finally { setCompletionProcessing(false); } };

    const handleRateMember = async (targetId: string, score: number) => { setRatingProcessing(targetId); try { await api.post(`/teams/${teamId}/rate`, { target_user_id: targetId, score, explanation: ratingExplanation }); setRatedMembers([...ratedMembers, targetId]); setRatingExplanation(""); alert("Rating submitted!"); } catch (e) { } finally { setRatingProcessing(null); } }
    ...
    const askAiForStack = async () => { setIsSuggesting(true); setSuggestions(null); try { const res = await api.post("/teams/suggest-stack", { description: team?.description || "", current_skills: localSkills }); if (res.data && (res.data.add.length > 0 || res.data.remove.length > 0)) setSuggestions(res.data); } catch (err) { console.error(err); } finally { setIsSuggesting(false); } };
    ...
    // --- REUSABLE TASK CARD ---
    const TaskCard = ({ task, isCompact = false }: { task: TaskItem, isCompact?: boolean }) => {
        const isMyTask = user && task.assignee_id === user.id;
        ...
        return (
            <div className={`border p-3 md:p-4 rounded-xl flex flex-col gap-3 transition-all ${statusColor} ${isCompact ? 'w-full' : ''}`}>
                <div className="flex items-start justify-between gap-3">
                    <div className="flex items-start gap-3 flex-1">
                        {!isCompact && (
                            <div className={`mt-1 p-2 rounded-full hidden md:block ${task.status === 'completed' ? 'bg-green-500/20 text-green-400' : task.status === 'rework' ? 'bg-red-500/20 text-red-400' : 'bg-gray-800 text-gray-400'}`}>
                        ...
                        <div>
                            {isCompact && (
                                <div className="flex items-center gap-2 mb-2">
                                    <img src={team?.members.find(m => m.id === task.assignee_id)?.avatar_url || "https://github.com/shadcn.png"} className="w-5 h-5 rounded-full" />
                                    <span className="text-[10px] text-gray-400 truncate max-w-[100px]">{team?.members.find(m => m.id === task.assignee_id)?.username}</span>
                                </div>
                            )}
                            <h4 className={`font-bold text-sm md:text-base ${task.status === 'completed' ? 'line-through text-gray-500' : 'text-white'}`}>{task.description}</h4>
                            <div className="flex flex-wrap gap-2 md:gap-4 mt-2 text-xs text-gray-400">
                                ...
                                {task.status === 'review' && !isMyTask && <span className="text-yellow-400 font-bold uppercase text-[10px] border border-yellow-400/30 px-1 rounded">Review</span>}
                            </div>
                        </div>
                    </div>
                    {isLeader && <button onClick={() => handleDeleteTask(task.id)} className="text-gray-600 hover:text-red-500"><Trash2 className="w-4 h-4" /></button>}
                </div>

                {/* Task Actions */}
                <div className="flex items-center gap-2 mt-auto pt-2 border-t border-white/5">
                    {(task.status === 'pending' || task.status === 'rework') && isMyTask && <button onClick={() => handleSubmitTask(task.id)} className="w-full bg-blue-600 hover:bg-blue-500 text-white py-1.5 rounded text-xs font-bold uppercase tracking-wider">Deliver</button>}
                    {task.status === 'review' && !isMyTask && (
                        <div className="flex gap-2 w-full">
                            <button onClick={() => handleVerifyTask(task.id)} className="flex-1 bg-green-600 hover:bg-green-500 text-white py-1.5 rounded text-xs font-bold flex items-center justify-center gap-1"><Check className="w-3 h-3" /> OK</button>
                            <button onClick={() => handleReworkTask(task.id)} className="flex-1 bg-red-600 hover:bg-red-500 text-white py-1.5 rounded text-xs font-bold flex items-center justify-center gap-1"><RotateCcw className="w-3 h-3" /> Fix</button>
                        </div>
                    )}
                    {task.is_overdue && isMyTask && !task.extension_active && <button onClick={() => openExtensionModal(task.id)} className="text-[9px] font-black text-blue-400 bg-blue-400/10 px-3 py-1 rounded border border-blue-400/20">Extend?</button>}
                    {task.extension_active && !isMyTask && <div className="flex gap-1 w-full justify-end"><span className="text-[10px] text-gray-500 my-auto mr-2">Extend Vote:</span><button onClick={() => handleVoteExtension(task.id, 'approve')} className="bg-green-600 p-1 rounded"><Check className="w-3 h-3" /></button><button onClick={() => handleVoteExtension(task.id, 'reject')} className="bg-red-600 p-1 rounded"><X className="w-3 h-3" /></button></div>}
                </div>
            </div>
        );
    };

    if (loading || !team || !user) return <div className="flex h-screen items-center justify-center bg-[#0B0E14] text-white"><Loader2 className="animate-spin text-purple-500" /></div>;
        ...
        <div className="flex min-h-screen bg-[#0B0E14] text-white overflow-hidden">
            ...
            <main className={`flex-1 transition-all duration-300 ${isCollapsed ? 'ml-20' : 'ml-64'} p-8`}>
                ...
                <div className="max-w-7xl mx-auto">
                    ...
                    {/* --- PROJECT WORKFLOW (BOARD / LIST) --- */}
                    {team.status !== 'planning' && team.status !== 'completed' && (
                        <div className="mb-12">
                            <div className="flex items-center justify-between mb-6">
                                <h2 className="text-2xl font-bold flex items-center gap-2"><ClipboardList className="text-blue-400" /> Project Workflow</h2>
                                
                                {/* VIEW TOGGLE */}
                                <div className="bg-gray-800 p-1 rounded-lg flex items-center gap-1 border border-gray-700">
                                    <button onClick={() => setViewMode('list')} className={`px-3 py-1.5 rounded-md flex items-center gap-2 text-sm font-bold transition-all ${viewMode === 'list' ? 'bg-gray-700 text-white shadow' : 'text-gray-400 hover:text-white'}`}><List className="w-4 h-4" /> List</button>
                                    <button onClick={() => setViewMode('board')} className={`px-3 py-1.5 rounded-md flex items-center gap-2 text-sm font-bold transition-all ${viewMode === 'board' ? 'bg-gray-700 text-white shadow' : 'text-gray-400 hover:text-white'}`}><LayoutDashboard className="w-4 h-4" /> Board</button>
                                </div>
                            </div>
                                    ...
                                    {team?.members.map(member => {
                                        ...
                                        return (
                                            <div key={member.id} className="bg-gray-900/30 border border-gray-800 rounded-xl p-4">
                                                ...
                                                <div className="space-y-3">{memberTasks.map(task => <TaskCard key={task.id} task={task} />)}</div>
                                            </div>
                                        );
                                    })}
                                    {tasks.length === 0 && <p className="text-gray-500 text-center italic">No tasks assigned yet.</p>}
                                </div>
                            )}
                            ...
                            {viewMode === 'board' && (
                                <div className="grid grid-cols-1 md:grid-cols-4 gap-6 overflow-x-auto pb-4">
                                    {['pending', 'rework', 'review', 'completed'].map(status => (
                                        <div key={status} className="flex flex-col gap-4 min-w-[280px]">
                                            <div className={`flex items-center gap-2 p-3 rounded-xl font-bold ${status === 'completed' ? 'bg-green-900/20 border border-green-500/30 text-green-300' : status === 'rework' ? 'bg-red-900/20 border border-red-500/30 text-red-300' : status === 'review' ? 'bg-yellow-900/20 border border-yellow-500/30 text-yellow-300' : 'bg-gray-900 border border-gray-800 text-gray-300'}`}>
                                                <div className={`w-2 h-2 rounded-full ${status === 'completed' ? 'bg-green-500' : status === 'rework' ? 'bg-red-500 animate-pulse' : status === 'review' ? 'bg-yellow-500' : 'bg-gray-500'}`}></div>
                                                <span className="uppercase">{status.replace('-', ' ')} ({tasks.filter(t => t.status === status).length})</span>
                                            </div>
                                            <div className="space-y-3">
                                                {tasks.filter(t => t.status === status).map(task => <TaskCard key={task.id} task={task} isCompact />)}
                                                {tasks.filter(t => t.status === status).length === 0 && <div className="h-20 border border-dashed border-gray-800 rounded-xl flex items-center justify-center text-gray-600 text-xs">Empty</div>}
                                            </div>
                                        </div>
                                    ))}
                                </div>
