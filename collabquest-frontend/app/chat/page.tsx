...
export default function ChatPage() {
    ...
    const handleSelectChat = async (targetId: string) => {
        try {
            ...
            setPendingAttachments([]); // Clear pending attachments on switch
    ...
    // ... [Keep existing handleHeaderClick, updateGroup, removeGroupMember, leaveGroup, blockGroup logic] ...
    ...
    // ... [Keep existing Add Member / Create Group Logic] ...
                ...
                <div className="hidden md:flex flex-1 bg-gray-900 border border-gray-800 rounded-2xl flex-col overflow-hidden relative h-full shadow-lg">
                    {activeChat ? (
                        <>
                                                    ...
                                                    <div className="space-y-2 mb-2">
                                                        {msg.attachments.map((att, idx) => (
                                                            <div key={idx} className="rounded-lg overflow-hidden bg-black/20">
                                                                {att.file_type === 'image' ? (
                                                                    <img src={att.url} alt="attachment" className="max-w-full h-auto rounded-lg max-h-60" />
                                                                ) : att.file_type === 'video' ? (
                                                                    ...
                                                                    <a href={att.url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-3 p-3 bg-white/10 hover:bg-white/20 transition">
                                    ...
                                    <div>
                                            ...
                                            <div className="flex gap-2 mb-2 overflow-x-auto pb-2">
                                                {pendingAttachments.map((att, i) => (
                                                    <div key={i} className="relative bg-gray-800 rounded-lg p-1 w-16 h-16 flex items-center justify-center shrink-0 border border-gray-700">
                                                        ...
                                                        {att.file_type === 'image' ? <img src={att.url} className="w-full h-full object-cover rounded" /> : 
                                                         att.file_type === 'video' ? <Film className="w-6 h-6 text-gray-400" /> :
                                                         att.file_type === 'audio' ? <Music className="w-6 h-6 text-gray-400" /> : <File className="w-6 h-6 text-gray-400" />}
                                                    </div>
                                                ))}
                                            </div>
                                        )}
                                        ...
                                        <div className="flex gap-2 items-center">
                                            ...
                                            <button 
                                                ...
                                                className={`p-3 bg-gray-800 hover:bg-gray-700 text-gray-400 rounded-xl transition ${isUploading ? 'animate-pulse opacity-50 cursor-not-allowed' : ''}`}
                                            ...
                                            <input 
                                                className="flex-1 bg-gray-950 border border-gray-800 rounded-xl px-4 py-3 outline-none focus:border-purple-500 transition" 
                                            ...
                                            <button 
                                                onClick={sendMessage} 
                                                className="bg-purple-600 hover:bg-purple-500 text-white p-3 rounded-xl transition disabled:opacity-50"
                            ...
                            {/* ... [Keep Group Info / Profile Info / Group Modal / etc. same as before] ... */}
                            {/* NOTE: I am omitting the modals code block here to save space as they remain unchanged. 
                                Ensure you keep the existing AnimatePresence blocks for GroupInfo, ProfileInfo, InfoModal, GroupModal, AddMemberModal. 
                            */}
                             <AnimatePresence>
